...
To get to the right answer, we need to hold `cognition` constant. In the right panel I show this by looking just at people with a low optimism score. Now if we compare happiness scores by road traveled, we get close to the correct answer (difference of 1.47).

:::{.column-margin}
Simulations involve a bit of noise.
:::
...
In practice we'd estimate the effect via a technique like multiple regression, which I'll show you here.[^inreality] You'll see that I modified the variable names to make @fig-roada easier to read. Most notably, the exposure road traveled is represented as `x` and the outcome happiness is represented as `y`. Cognition and active are simply `c` and `a`, respectively.
...
Talk about c-c closing back doors and how experiments do this so well. Then talk about quasi.
...
::::{.content-hidden unless-format="html"}
::: {.callout-note icon=false}
## **Two important notes about internal validity**

First, internal validity is not a property of your research design per se; it's a characteristic of your claim. While it's true that some designs, like randomized controlled trials, face fewer threats to internal validity in theory, it's also true that study implementation matters. The RCT label does not insulate a poorly conducted RCT from criticism. It's best to think consider the internal validity of claims on a study-by-study basis. Second, there is not a statistical test that will tell you if your claim has high internal validity. You can use the results of various statistical tests to probe the support for your causal claim, but no test will tell you that a claim has high or low internal validity. P-values, for instance, are silent on the issue.
:::
:::: 
