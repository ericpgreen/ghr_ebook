...
df_road_A_rd_2 <- df_road_A_rd %>%
  ggplot(aes(x = days, y = happiness_obs, color = road_traveled)) +
    ...
    labs(x = "Days from birth to cutoff (Jan 1, 1980)", y = "Happiness", 
         color = "Road traveled",
         subtitle = str_wrap("Taking the road less traveled increased happiness by an estimated 1.5 points", 70)) + 
    guides(color = FALSE) +
    theme_ghr() +
...
The key to this design, and others in this category, is that the instrument changes the probability of exposure (treatment) WITHOUT having any other mechanism of impacting the outcome [@matthay:2020b]. In a later chapter we'll see examples of regression discontinuity in practice, along with other designs like instrumental variables, difference-in-differences, and interrupted time series.
